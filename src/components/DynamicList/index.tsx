...
export const DynamicList = ({
  ...
  const [isNotLikePhone, setIsNotLikePhone] = useState<boolean>();
  ...
  return <Popover className="relative p-4">
        ...
        <Popover.Panel className="absolute z-10 mt-1">
          <div className="bg-white rounded-md shadow-md flex flex-col w-full md:w-72">
            ...
            dark:focus:ring-blue-500 duration-100 `}>
              <input
                type="text"
                value={inputValue} onChange={handleOnInputChange} onKeyDown={handleOnKeyPress}
                className={`pl-6 pr-6 py-3 bg-transparent outline-none placeholder-slate-300 dark:placeholder-zinc-400`}
                placeholder="Type" />


              <div className="flex items-center pr-3">
                <span className={`text-slate-400 bg-slate-200 uppercase font-bold  px-2 shadow-sm rounded-full `} style={{ fontSize: "10px" }}>
                  enter
                </span>
              </div>
                    ...
                    filteredValues.map((value: string, index: number) =>
                      <li key={index} className="flex select-none px-4 py-4 justify-between hover:bg-zinc-100 text-zinc-700">
                        <div dangerouslySetInnerHTML={{
                          __html:
                            inputValue ?
                              value.replace(inputValue, `<span ${isDuplicate ? '' : 'class="text-yellow-700 font-bold px-1 py-0.5 rounded-md bg-yellow-100"'} >${inputValue}</span>`)
                              : value
                        }}></div>
